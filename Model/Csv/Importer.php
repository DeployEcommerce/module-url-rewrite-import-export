...
use DeployEcommerce\UrlRewriteImportExport\Model\UrlRewrite\DuplicateAutogeneratedFixer;
...
class Importer
{
    ...
    public function __construct(
        ...
        private readonly DuplicateAutogeneratedFixer $duplicateFixer
    ) {
    }
    ...
    public function import(
        ...
        // A CSV can carry a superseded generated path that no longer collides with anything
        // on (request_path, store_id), so the insert succeeds and leaves two rows claiming
        // is_autogenerated = 1 for one entity — which makes category/product collections
        // throw on load. Demote the stale ones before handing control back.
        try {
            $result->addDemoted($this->duplicateFixer->fix());
        } catch (\Throwable $fixError) {
            $result->addError(sprintf(
                'Rows were imported, but the duplicate autogenerated rewrite check failed: %s',
                $fixError->getMessage()
            ));
        }
